"""
FeedVision — İzleme + Kontrol Kriterleri (madde 1: ROI kriter mantığı,
madde 2: güvenlik interlock, madde 7: aralık dışı alarm — üçü TEK motor)

Ne yapar: operatörün tanımladığı kriterler (ör. "ui_screen/basinc ROI'si
2-6 bar aralığında olmalı") ile o an okunan değerleri (ROI OCR metni ya da
STM32 bağlantı durumu) karşılaştırır, ihlal edilenleri döner. main.py bu
ihlalleri kullanarak motoru durdurur + UI'da alarm gösterir (bkz. main.py'deki
periyodik kontrol döngüsü).

Neden tek motor: ROI kriter mantığı, motor-durduran interlock ve "aralık
dışı değer alarmı" aslında aynı sorunun üç yüzü — "izlenen bir değerin
kriteri var, ihlal olunca ne olur" sorusu. Ayrı ayrı yazmak tutarsız
davranışa yol açardı (ör. interlock'un baktığı eşikle alarmın baktığı eşik
farklı yerlerde tanımlı olabilirdi).

İki kriter kaynağı (15-09-2026 kapsam kararı):
- "roi": bir kameranın ROI'sinden OCR ile okunan sayı, min/max aralığı.
- "stm": STM32 BAĞLANTISININ ne kadar süredir sessiz kaldığı (timeout_s).
  Protokolde (docs/protocol.md) STM32'nin ayrı bir hata/fault alanı YOK —
  bu yüzden şimdilik STM kaynaklı kriter SADECE "bağlantı koptu/zaman
  aşımı" ile sınırlı. Daha gelişmiş stall tespiti (running=1 iken
  remaining'in beklenmedik şekilde sabit kalması vb.) kasıtlı olarak
  YAPILMADI — backlog'a ayrı not düşüldü.

Bu dosya SADECE saf değerlendirme mantığını içerir — kamera/seri port G/Ç'si
main.py'de kalıyor, bu yüzden donanımsız test edilebilir.
"""

from dataclasses import dataclass
from typing import Any


@dataclass
class RuleViolation:
    """Bir kriterin ihlal edildiği anki bilgisi — UI banner'ı ve motor
    durdurma kararı bu nesneden besleniyor."""

    rule_id: str
    rule_name: str
    value: float
    min: float | None
    max: float | None
    stop_motor: bool
    source_label: str  # ör. "ui_screen / basinc" ya da "stm / bağlantı" — kullanıcıya gösterilecek kısa açıklama


@dataclass
class RuleEvalSkipped:
    """Bir kriter DEĞERLENDİRİLEMEDİ (ihlal değil) — ör. OCR metni sayıya
    çevrilemedi, ya da STM32'den henüz hiç durum satırı gelmedi.

    Neden ihlal SAYILMIYOR: okunamayan bir değeri "aralık dışı" sanıp motoru
    durdurmak, gerçek bir arızadan daha kötü bir yanlış pozitif olurdu (bkz.
    proje notu: "sessizce hatalı veri üretmek, hiç okumamaktan daha kötü").
    Bunun yerine ayrı bir "okunamadı" listesi olarak taşınır, operatör
    görebilir ama motor durmaz.
    """

    rule_id: str
    rule_name: str
    reason: str


def _try_parse_number(text: str) -> float | None:
    """OCR'dan gelen serbest metinden bir ondalık sayı çıkarmayı dener.

    Tesseract çoğu zaman etrafında birim/boşluk/gürültü karakteriyle birlikte
    metin döner (ör. "23.4 C", " 5,2bar") — burada sadece basit bir temizlik
    yapılıyor (virgül->nokta, izin verilmeyen karakterleri at). Karmaşık/
    çok-sayılı metinlerde (ör. "12:30") yanlış sayı çıkarma riskini azaltmak
    için TÜM metin sayıya çevrilemiyorsa None dönülür — kısmi/tahmin ile
    sayı üretilmez (yanlış pozitif riski, bkz. RuleEvalSkipped notu).
    """
    cleaned = text.strip().replace(",", ".")
    if not cleaned:
        return None
    try:
        return float(cleaned)
    except ValueError:
        return None


def _evaluate_roi_rule(
    rule: dict[str, Any], roi_readings: dict
) -> tuple[float | None, str | None]:
    """ROI kaynaklı bir kriter için o anki değeri okur.

    roi_readings: main.py'nin ürettiği {(cam_id, roi_name): {"text": ..., "avg_color_hsv": ...}} sözlüğü.
    Döner: (deger, hata_mesaji) — deger bulunduysa hata_mesaji None, aksi halde deger None.
    """
    key = (rule.get("cam_id"), rule.get("roi_name"))
    reading = roi_readings.get(key)
    if reading is None:
        return None, f"ROI '{rule.get('roi_name')}' bu karede okunmadı (kamera/ROI eşleşmedi)"
    text = reading.get("text", "")
    value = _try_parse_number(text)
    if value is None:
        return None, f"OCR metni sayı değil: \"{text}\""
    return value, None


def _evaluate_stm_rule(
    rule: dict[str, Any], stm32_meta: dict
) -> tuple[float | None, str | None]:
    """STM kaynaklı "bağlantı koptu/timeout" kriteri için o anki değeri (son
    durum satırının üzerinden geçen saniye) okur.

    stm32_meta: {"is_connected": bool, "status_age_s": float | None}
    (main.py, bridge.is_connected + bridge.get_status_age()'den kurar).
    """
    timeout_s = rule.get("timeout_s")
    if timeout_s is None:
        return None, "timeout_s tanımlanmamış (kriter eksik yapılandırılmış)"
    if not stm32_meta.get("is_connected", False):
        # Bağlantı hiç yoksa "sonsuz" beklemiş sayılır — timeout'un kendisini
        # aşmış kabul edilir (deger olarak timeout_s+1 donduruluyor, sadece
        # "esik asildi" gorunumu icin, gercek bir olcum degil).
        return float(timeout_s) + 1.0, None
    age = stm32_meta.get("status_age_s")
    if age is None:
        return None, "STM32'den henüz hiç durum satırı gelmedi"
    return float(age), None


def _source_label(rule: dict[str, Any]) -> str:
    if rule.get("source") == "roi":
        return f"{rule.get('cam_id')} / {rule.get('roi_name')}"
    return "stm / bağlantı"


def evaluate_rules(
    rules: list[dict[str, Any]],
    roi_readings: dict,
    stm32_status: dict,
    stm32_meta: dict | None = None,
) -> tuple[list[RuleViolation], list[RuleEvalSkipped]]:
    """Tüm AKTİF (enabled=True) kriterleri değerlendirir.

    stm32_status şu an sadece geriye uyumluluk/ileride genişleme için
    parametre listesinde tutuluyor (STM kaynaklı kriterler artık
    stm32_meta'yı kullanıyor, ham durum sözlüğünü değil — bkz. modül
    docstring'indeki kapsam kararı).

    Döner: (ihlaller, atlananlar). İhlaller motor durdurma + alarm için
    kullanılır; atlananlar sadece bilgi amaçlı (operatöre "şu kriter şu an
    okunamıyor" göstermek için) — motor kararını ETKİLEMEZ.
    """
    del stm32_status  # bkz. docstring — su an kullanilmiyor, imza uyumu icin duruyor
    stm32_meta = stm32_meta or {}
    violations: list[RuleViolation] = []
    skipped: list[RuleEvalSkipped] = []

    for rule in rules:
        if not rule.get("enabled", True):
            continue
        rule_id = rule.get("id", "")
        rule_name = rule.get("name", rule_id)
        source = rule.get("source")

        if source == "roi":
            value, error = _evaluate_roi_rule(rule, roi_readings)
            min_v = rule.get("min")
            max_v = rule.get("max")
        elif source == "stm":
            value, error = _evaluate_stm_rule(rule, stm32_meta)
            min_v = None
            max_v = rule.get("timeout_s")
        else:
            value, error = None, f"Bilinmeyen kriter kaynağı: {source!r}"
            min_v = max_v = None

        if value is None:
            skipped.append(RuleEvalSkipped(rule_id=rule_id, rule_name=rule_name, reason=error or "bilinmeyen hata"))
            continue

        out_of_range = (min_v is not None and value < min_v) or (max_v is not None and value > max_v)
        if out_of_range:
            violations.append(
                RuleViolation(
                    rule_id=rule_id,
                    rule_name=rule_name,
                    value=value,
                    min=min_v,
                    max=max_v,
                    stop_motor=bool(rule.get("stop_motor", False)),
                    source_label=_source_label(rule),
                )
            )

    return violations, skipped
