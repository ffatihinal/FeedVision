...
> Bu dosya, web app'teki "ℹ️ Konsol Rehberi" pop-up'ının **son kullanıcıya sadeleştirilmeden önceki** hâlidir — geliştirici/bakım referansı olarak burada tutulur. Kullanıcıya gösterilen pop-up daha sade bir dille yazılmıştır (`ui/admin.html`).
...
|---|---|---|---|---|
| 🎥 Kamera (Chamber/UI Screen) | `vision.py`, gerçek Picamera2 donanım denemesi | Kamera butonuna basınca | `/vision/{id}/snapshot` isteğinin gerçek HTTP cevabı — başarılıysa content-type/uzunluk, başarısızsa backend'in gerçek hata sebebi (`vision.errors`) | — |
...
---
*Kaynak: 2026-09-09 tarihli picamera2/kamera-Konsol düzeltmesi oturumu. Web app'teki pop-up içeriği son kullanıcı için ayrıca sadeleştirilmiştir — güncel kullanıcı metni için `ui/admin.html`'e bakılmalı.*
