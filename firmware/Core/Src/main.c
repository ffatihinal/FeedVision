...
static void     enc_baslat(encoder_t *e, TIM_HandleTypeDef *htim);  // encoder'ı sıfırdan başlatır (donanım sayacını çalıştırır)
static void     enc_guncelle(encoder_t *e);                          // sayacı okuyup toplam mesafeye ekler (periyodik çağrılır)
static void     enc_sifirla(encoder_t *e);                           // sayacı ve toplamı 0'a resetler
static int32_t  enc_mikrometre(int32_t sayim);                       // ham sayımı mikrometreye (mm'nin 1000'de biri) çevirir

/* --- Step motor fonksiyonları --- */
static void     step_baslat(uint8_t yon, uint32_t adet, uint32_t delay_us);  // step motoru başlatır: yön + kaç adım + ne hızda
static void     step_durdur(void);                                            // step motoru anında durdurur

/* --- DC motor fonksiyonu --- */
static void     dc_ayarla(uint8_t durum);  // DC motoru ileri/geri/dur durumuna sokar

/* --- Seri port (UART) fonksiyonları --- */
static void     uart_gonder(const char *s);           // bir metni Mac'e (seri port üzerinden) gönderir
static void     durum_gonder(void);                    // encoder/motor durumunu JSON olarak Mac'e yollar
static void     komut_isle(const char *satir);         // Mac'ten gelen bir JSON komut satırını yorumlayıp uygular

/* --- JSON okuma yardımcıları (kütüphanesiz, elle yazılmış basit ayrıştırıcı) --- */
static int      json_int_oku(const char *json, const char *anahtar, int32_t *cikti);                  // JSON'dan bir sayı değeri çeker (örn: "delay":500)
static int      json_str_oku(const char *json, const char *anahtar, char *cikti, uint16_t cikti_boyut); // JSON'dan bir metin değeri çeker (örn: "cmd":"step")
